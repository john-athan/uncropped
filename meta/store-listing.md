...
The store icon is **not** `icons/icon128.png`. The toolbar icon fills its frame,
which is right for a toolbar and wrong here: Google's image guidelines put 96 px
of artwork in the middle of the 128 px store icon and leave the surrounding
16 px transparent, because the console draws its own rounding and shadow in that
margin. `screenshots/store-icon.png` is the padded one, and it is what the
"Store icon" field wants.

| Asset | Size | File |
|---|---|---|
| Store icon | 128 x 128 | `screenshots/store-icon.png` |
