...
job rather than a writing job. Fill the checklist at the bottom before you press
publish.
...
- [x] Search the Chrome Web Store for "Uncropped" and confirm the name is free in
      this category. Done 2026-09-01, clear, recorded in THIRD_PARTY.md.
...
- [ ] Publish `docs/` to GitHub Pages so the privacy policy URL resolves. The
      store rejects a policy URL that 404s.
- [ ] Register the developer account and pay the one time fee.
- [ ] Set a developer contact email and verify it. This is mandatory.
- [x] Build the package: `tools/package.sh`. Produces a 20 KB, 13 file zip.
- [ ] Load the zip unpacked one last time and take a capture with it on each of
      the three shapes in `tests/pages/`: a plain article, an app shell whose
      document does not scroll, and a page with a wide table.
- [ ] Replace the illustrative store screenshots with captures from the shipped
      build if the UI changes before release.
- [ ] Tag `v1.0.0`, which runs the provenance workflow.
